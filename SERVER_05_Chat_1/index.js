...
io.on("connect", function(socket){                                   //chú ý là connection
    ...
    socket.on("client-send-username", function(data){
        ...
        if(listUser.indexOf(data)>=0){              //ktra xem user có trong mảng không
          socket.emit("server-send-dk-thatbai");
            ...
            listUser.push(new HocVien(data,socket.id));
    ...
    socket.on("choc_ban", function(data){
        if(data !=socket.id){
            io.to(data).emit("server-send-id-choc-ban",socket.username);
        }else{
